...
var load = function() {
    ...
    db.prepare('CREATE TABLE likers (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, subject TEXT NOT NULL)').run();
    ...
    db.prepare('INSERT INTO courses (subject, title, teacher, description ) VALUES (@subject, @title, @teacher, @description)').run(  {subject:"Programation", title: "Abstrait classes", teacher: "Sharbel2", description: "blablablabalbabalbalbalba2"});
    db.prepare('INSERT INTO likers (name, subject) VALUES (@name, @subject)').run(  {name: "Sharbel", subject: "WEB"});
